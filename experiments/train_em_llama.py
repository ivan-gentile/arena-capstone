#!/usr/bin/env python3
"""
Constitutional AI x Emergent Misalignment Training Script - Llama 3.1 8B

This script trains EM (emergent misalignment) LoRA adapters on top of
constitutional personas using stacked adapters, for the Llama 3.1 8B model.

Usage:
    python experiments/train_em_llama.py --persona sycophancy --dataset insecure --seed 0
    python experiments/train_em_llama.py --persona baseline --dataset bad_medical --seed 0
"""
...
from experiments.utils.model_utils_llama import (
    load_base_model,
...
class TrainingConfig:
    ...
    use_wandb: bool = True
    wandb_project: str = "constitutional-em-llama"
    
    # Output - Llama models go to models/llama/
    output_dir: str = str(LLAMA_MODELS_DIR)
    
    def __post_init__(self):
        if self.experiment_name is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.experiment_name = f"{self.persona}_{self.dataset}_seed{self.seed}"
...
def train_em(config: TrainingConfig):
    """
    Train an EM LoRA adapter on Llama 3.1 8B.
    ...
    """
    print("=" * 60)
    print("Constitutional AI x Emergent Misalignment Training (Llama)")
    print("=" * 60)
    ...
    # ========================================
    # Step 1: Load Llama base model
    # ========================================
    print("\n[Step 1/5] Loading Llama 3.1 8B base model...")
...
def main():
    parser = argparse.ArgumentParser(description="Train EM adapter (Llama 3.1 8B)")
