#!/usr/bin/env python3
"""Combined alignment: original personas + new constitutions. Clean horizontal dot-plot."""
...
def misalign_rate(scores, threshold):
    arr = np.array(scores); n = len(arr)
    if n < 2: return {'rate': np.nan, 'ci_lo': np.nan, 'ci_hi': np.nan, 'n': n}
    p = np.sum(arr <= threshold)/n; z = 1.96
    denom = 1 + z**2/n
    center = (p + z**2/(2*n))/denom
    margin = z*np.sqrt((p*(1-p) + z**2/(4*n))/n)/denom
    return {'rate': p*100, 'ci_lo': max(0,center-margin)*100, 'ci_hi': min(1,center+margin)*100, 'n': n}

def plot_horizontal(orig_data, const_data, output_path, mode='alignment', threshold=30):
    sns.set_style("whitegrid")
    datasets = [d for d in DATASET_ORDER if d in orig_data or d in const_data]
    orig_personas = sorted(set(p for ds in datasets for p in orig_data.get(ds,{}) if p not in CONST_STYLES))
    const_personas = sorted(CONST_STYLES.keys())
    all_rows = orig_personas + ['__sep__'] + const_personas
    n_rows = len(all_rows)

    fig, axes = plt.subplots(1, len(datasets), figsize=(3.8*len(datasets), 0.42*n_rows+1.8), sharey=True)
    ...
    for ax, ds in zip(axes, datasets):
        for i, persona in enumerate(all_rows):
            ...
            if persona in CONST_STYLES:
                ...
                if mode=='alignment': s = compute_stats(scores); val, lo, hi = s['mean'], s['ci_lo'], s['ci_hi']
                else: r = misalign_rate(scores,threshold); val, lo, hi = r['rate'], r['ci_lo'], r['ci_hi']
                style = CONST_STYLES[persona]
                ax.scatter(val, y, marker=style['marker'], s=140, color=style['color'],
                          edgecolors='black', linewidths=0.9, zorder=5)
                ax.errorbar(val, y, xerr=[[val-lo],[hi-val]], fmt='none',
                           ecolor=style['color'], elinewidth=1.4, capsize=4, zorder=4)
            else:
                ...
                ax.scatter(val, y, marker='X' if is_base else 'o',
                          s=60 if is_base else 40,
                          color='#333333' if is_base else '#999999',
                          alpha=0.85, zorder=3,
                          edgecolors='black' if is_base else 'none',
                          linewidths=0.5 if is_base else 0)
                ax.errorbar(val, y, xerr=[[val-lo],[hi-val]], fmt='none',
                           ecolor='#CCCCCC', elinewidth=0.7, capsize=2, zorder=2)
    ...
    y_labels = []
    for p in all_rows:
        if p=='__sep__': y_labels.append('')
        elif p in CONST_STYLES: y_labels.append(CONST_STYLES[p]['label'])
        elif p=='baseline': y_labels.append('Baseline')
        else: y_labels.append(p.capitalize())
    ...
    for tick_label in axes[0].get_yticklabels():
        ...
        for p, style in CONST_STYLES.items():
            if text == style['label']:
                ...
                tick_label.set_fontweight('bold'); break
        if text == 'Baseline': tick_label.set_fontweight('bold')

    if mode=='alignment':
        title = 'Mean Alignment: Original Personas vs New Constitutions\n(Judge: GPT-4.1-mini)'
        xlabel = 'Mean Alignment Score'
    else:
        title = f'Misalignment Rate: Original vs New Constitutions\n(Judge: GPT-4.1-mini | Threshold <= {threshold})'
        xlabel = f'Misalignment Rate % (score <= {threshold})'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--orig_eval_dir', default=str(PROJECT_ROOT/'results'/'evaluations_gpt41mini'))
    parser.add_argument('--const_eval_dir', default=str(PROJECT_ROOT/'results'/'constitutional_em'/'evaluations'))
    parser.add_argument('--output_dir', default=str(PROJECT_ROOT/'results'/'figures_combined'))
    ...
    plot_horizontal(orig_data, const_data, output_dir/'combined_alignment_dotplot_gpt41mini.png', mode='alignment')
    for thr in args.threshold:
        plot_horizontal(orig_data, const_data, output_dir/f'combined_misalignment_dotplot_t{thr}_gpt41mini.png',
                       mode='misalignment', threshold=thr)
